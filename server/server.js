...
const serviceAccountKey = require("./blog-chronicle-firebase-adminsdk-lp3x6-b1a2c2622e.json");
...
// Initialize Firebase Admin once for both Auth and Storage
admin.initializeApp({
  credential: admin.credential.cert(serviceAccountKey),
  storageBucket: "gs://blog-chronicle.appspot.com",
});
...
// Your existing generateUploadURL function
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    cb(null, `${nanoid()}-${Date.now()}${path.extname(file.originalname)}`);
  },
});
...
server.post("/upload-image", upload.single("image"), (req, res) => {
  res.json({
    url: `${process.env.SERVER_DOMAIN}/uploads/${req.file.filename}`,
  });
});
...
server.post("/signup", (req, res) => {
  ...
  if (name.length < 3) {
    return res
      .status(403)
      .json({ error: "name must be at least 3 letters long" });
  }
  if (!email.length) {
...
server.post("/get-blog", (req, res) => {
  ...
  Blog.findOneAndUpdate(
    ...
    .populate(
      "author",
      "personal_info.profile_img personal_info.username personal_info.name"
    )
